(self.AMP=self.AMP||[]).push({n:"amp-analytics",v:"1910212134200",f:(function(AMP,_){
...
function (_AMP$BaseElement) {
  ...
  _proto.allowParentPostMessage_ = function allowParentPostMessage_() {
    ...
    if (this.isInFie_ == null) {
      this.isInFie_ = (0, _iframeHelper.isInFie)(this.element);
    }
 ...
 * limitations under the License.
 */
var COMSCORE_CONFIG = "{\"vars\":{\"c2\":\"1000001\"},\"requests\":{\"host\":\"https://sb.scorecardresearch.com\",\"base\":\"${host}/b?\",\"pageview\":\"${base}c1=2&c2=${c2}&cs_ucfr=$IF($EQUALS(${consentState}, sufficient), 1)$IF($EQUALS(${consentState}, insufficient), 0)$IF($EQUALS(${consentState}, ), )&cs_amp_consent=${consentState}&cs_pv=${pageViewId}&c12=${clientId(comScore)}&rn=${random}&c8=${title}&c7=${canonicalUrl}&c9=${documentReferrer}&cs_c7amp=${ampdocUrl}\"},\"triggers\":{\"defaultPageview\":{\"on\":\"visible\",\"request\":\"pageview\"}},\"transport\":{\"beacon\":false,\"xhrpost\":false,\"image\":true}}";
 ...
 * limitations under the License.
 */
var INFONLINE_CONFIG = "{\"vars\":{\"sv\":\"ke\",\"ap\":\"1\"},\"transport\":{\"beacon\":false,\"xhrpost\":false,\"image\":true},\"requests\":{\"pageview\":\"${url}?st=${st}&sv=${sv}&ap=${ap}&co=${co}&cp=${cp}&ps=${ps}&host=${canonicalHost}&path=${canonicalPath}&type=pageview\",\"event\":\"${url}?st=${st}&ev=${ev}&sv=${sv}&ap=${ap}&co=${co}&cp=${cp}&ps=${ps}&host=${canonicalHost}&path=${canonicalPath}&type=event\"},\"triggers\":{\"pageview\":{\"on\":\"visible\",\"request\":\"pageview\"}}}";
